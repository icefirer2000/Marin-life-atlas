...
- 数据集：v1.8 内置代表性鱼类档案与年度栖息地节点
- 覆盖物种：黄鳍金枪鱼、大西洋鳕、蓝鲨、鲸鲨、太平洋沙丁鱼
- 覆盖年份：2000、2005、2010、2015、2020、2024
- 用途：验证鱼类节点显示、图鉴面板、年份筛选、独立起止年份轨迹和箭头路线交互
- 数据性质：示例数据结构，不作为科研结论或真实个体迁徙路线使用
- 记录日期：2026-09-02（UTC+8）

v1.8 起的轨迹线表示“年度栖息地节点变化线”。如果节点来自 OBIS、GBIF 或调查数据，它只能表达物种分布中心或热点变化；只有接入卫星标记、声学遥测、标记放流等个体追踪数据后，才应标注为真实迁徙路线。v1.9 的避陆箭头轨迹为几何展示启发式，不代表真实海流、洄游生态或个体追踪证据。

后续真实数据接入建议：

- OBIS occurrence records：适合海洋物种出现记录和年份分布筛选
- GBIF occurrence records：适合补充全球生物多样性出现记录
- NOAA DisMAP：适合美国海域鱼类分布模型与多年变化指标
- ICES DATRAS：适合欧洲海域长期渔业调查
- Movebank / OTN / U.S. Animal Telemetry Network：适合真实个体迁徙或遥测路线
